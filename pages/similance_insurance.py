...
try:
    ...
    def get_insurance_retrieved_df(retriever, val_df, pd):
        try:
            ...
            logger.error(f"Error in get_insurance_retrieved_df: {str(e)}")
            raise e
    ...
    def generate_look_alike_insurance(pandas_df, k):
        try:
            ...
            logger.error(f"Error in generate_look_alike_insurance: {str(e)}")
            raise e
            ...
            with st.form('fileform'):
                ...
                if submitted:
                    try:
                        ...
                        logger.error(f"AttributeError in insurance_generate_form: {str(e)}")
                        st.error("Please submit the uploaded file.")
                    except Exception as e:
                        logger.error(f"Unexpected error in insurance_generate_form: {str(e)}")
                        st.error(f"An unexpected error occurred: {str(e)}")
        except Exception as e:
            logger.error(f"Error in insurance_generate_form: {str(e)}")
    ...
    logger.info("Similance insurance page loaded successfully")
